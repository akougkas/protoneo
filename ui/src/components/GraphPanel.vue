<template>
  <div class="graph-panel">
    <div class="panel-header">
      <span class="panel-title">Paper Knowledge Graph</span>
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'Building Paper Knowledge Graph...' : 'Updating...' }}
        </div>
        ...
        <div v-if="selectedItem" class="detail-panel">
          ...
          <div v-if="selectedItem.type === 'node'" class="detail-content">
            ...
            <div class="detail-row" v-if="selectedItem.data.attributes?.description">
              <span class="detail-label">Description:</span>
              <span class="detail-value">{{ selectedItem.data.attributes.description }}</span>
            </div>
            <div class="detail-row" v-if="selectedItem.data.attributes?.source_section">
              <span class="detail-label">Section:</span>
              <span class="detail-value">{{ selectedItem.data.attributes.source_section }}</span>
            </div>
            <div class="detail-row" v-if="selectedItem.data.attributes?.confidence && selectedItem.data.attributes.confidence < 1">
              <span class="detail-label">Confidence:</span>
              <span class="detail-value" :style="{ color: selectedItem.data.attributes.confidence < 0.5 ? '#c44' : '#666' }">
                {{ Math.round(selectedItem.data.attributes.confidence * 100) }}%
              </span>
            </div>
      ...
      <div v-else class="graph-state">
        ...
        <p class="empty-text">{{ isSimulating ? 'Waiting for ontology generation...' : 'No graph data available' }}</p>
      </div>
    </div>
...
const showEdgeLabels = ref(true) // Default to show edge labels
...
// Watch isSimulating changes, detect simulation finish
...
// Deterministic colors by entity type. Semantic types get vivid colors,
// structural types get muted tones. Same type always gets the same color.
const ENTITY_TYPE_COLORS = {
  // Structural (muted)
  Paper: '#2C3E50', Section: '#7F8C8D', Diagram: '#95A5A6', Table: '#95A5A6',
  Equation: '#8E44AD', Reference: '#BDC3C7',
  // Core semantic (vivid)
  Claim: '#E74C3C', Method: '#3498DB', Dataset: '#27AE60', Metric: '#F39C12',
  Baseline: '#E67E22', Result: '#2ECC71', Limitation: '#C0392B', Concept: '#1ABC9C',
}
// Hash-based fallback for LLM-generated types
const _FALLBACK_COLORS = ['#6C5CE7','#00B894','#FDCB6E','#E17055','#0984E3','#D63031','#00CEC9','#636E72']
function typeColor(typeName) {
  if (ENTITY_TYPE_COLORS[typeName]) return ENTITY_TYPE_COLORS[typeName]
  let hash = 0
  for (let i = 0; i < typeName.length; i++) hash = ((hash << 5) - hash) + typeName.charCodeAt(i)
  return _FALLBACK_COLORS[Math.abs(hash) % _FALLBACK_COLORS.length]
}
...
const entityTypes = computed(() => {
  ...
  props.graphData.nodes.forEach(node => {
    ...
    if (!typeMap[type]) {
      typeMap[type] = { name: type, count: 0, color: typeColor(type) }
    }
    typeMap[type].count++
  })
...
const renderGraph = () => {
  ...
  // Process edge data, calculate number and index of edges between same pair of nodes
  const edgePairCount = {}
  const selfLoopEdges = {} // Grouped self loop edges by node
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // Curvature range increases with number of edges
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // If edge direction is opposite, flip curvature
      // This ensures all edges are distributed in the same frame of reference
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Color scale
  const getColor = (type) => typeColor(type)
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravity towards the center to cluster independent node groups
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw an arc from node and return
      const loopRadius = 30
      // Start from right side of node, go around and come back
    ...
    // Calculate curve control point - dynamically adjust based on number of edges and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Offset perpendicular to line direction, calculated based on distance ratio
    // More edges = larger ratio of offset to distance
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .enter().append('circle')
    .attr('r', d => {
      if (d.type === 'Paper') return 14
      const structural = ['Section', 'Diagram', 'Table', 'Reference', 'Equation']
      return structural.includes(d.type) ? 6 : 10
    })
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record position, do not restart simulation (distinguish click and drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect if truly started dragging (movement exceeds threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Only restart simulation when true drag is detected for the first time
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only gradually stop simulation if truly dragged
        if (d._isDragging) {
  ...
  nodeGroup.selectAll('circle').each(function(d) {
    const desc = d.rawData?.attributes?.description || ''
    const sec = d.rawData?.source_section || d.rawData?.attributes?.source_section || ''
    let tip = `${d.name} (${d.type})`
    if (desc) tip += `\n${desc}`
    if (sec) tip += `\nSection: ${sec}`
...
/* Hint after simulation finishes styles */
