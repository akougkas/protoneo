...
def _find_candidate_pairs(entities: list) -> list[tuple[str, str]]:
    """Fast pre-pass: find entity pairs with high token overlap."""
    pairs = []
    ...
    for i in range(len(names)):
        for j in range(i + 1, len(names)):
            ...
            if jaccard > 0.5 or is_substring:
                pairs.append((label_i, label_j))
    return pairs[:50]  # Cap at 50 pairs to keep prompt manageable
...
async def resolve_coreferences(
    ...
    if candidate_pairs:
        pair_list = "\n".join(
            f"- \"{a}\" vs \"{b}\"" for a, b in candidate_pairs
        )
        prompt = _COREF_PAIRS_PROMPT.format(pair_list=pair_list)
        logger.info(
            "Co-ref pre-pass found %d candidate pairs from %d entities",
            len(candidate_pairs), len(entities),
        )
    else:
        compact = entities[:40]
        entity_list = "\n".join(
        ...
        prompt = _COREF_PROMPT.format(entity_list=entity_list)
        logger.info(
            "No candidate pairs found; sending %d/%d entities for abbreviation detection",
            len(compact), len(entities),
        )
    ...
    try:
        ...
        result = _parse_coref_response(response.content)
    except Exception as e:
        logger.warning("Co-reference resolution LLM call failed: %s", e)
        return {"merged": 0, "aliases_created": 0,
                "total_entities_before": len(entities),
                "total_entities_after": len(entities)}
