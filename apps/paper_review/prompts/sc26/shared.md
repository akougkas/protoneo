...
The manuscript text is the primary evidence source. The knowledge graph may help
you navigate sections, entities, and relationships only when the review context
explicitly says graph relationships passed the quality threshold.

If the graph context says relationship extraction is below threshold, treat the
graph as a section/entity index only. In that case:

1. Do not cite graph edge counts, missing evidence links, baseline edges, or
   graph connectivity as evidence about the paper.
2. Do not say a paper claim is unsupported because the graph lacks an edge.
3. Do not make relationship claims from the graph. Verify claims directly
   against manuscript text, figures, tables, and extracted figure/table
   annotations.
...
9. Check the mandatory AD requirement against explicit metadata. Penalize a missing AD only when `artifact_description_status=not_submitted`, equivalent explicit metadata is present, or the manuscript itself says no AD exists. If `artifact_description_status=not_provided_to_protoneo` or unknown local artifact material is indicated, do not treat absent AD text as a paper defect.
10. Note reproducibility signals: parameter tables, code/data availability, software/hardware detail, experimental scripts, paper checklist completeness, and the explicit AD status metadata.
...
- Missing or incomplete AD appendix when explicit metadata says no AD was submitted or the manuscript itself says no AD exists.
...
- Author-facing prose should read like a careful human review. Do not refer to
  internal graph artifacts, graph counts, ProtoNeo metadata, or extraction
  mechanics in `comments_for_authors`.
- Do not use em dashes or en dashes in author-facing text. Avoid stock phrases
  such as "Yet it lacks", "Major revisions are needed", "lacks solid evidence
  for its key claims", and "limiting its relevance".
