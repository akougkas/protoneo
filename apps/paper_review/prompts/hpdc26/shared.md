...
2. Cite specific figure analysis when discussing experimental evidence (e.g., "Figure 3 shows a linear scaling trend up to 64 nodes").
...
- `2 = Weak reject`: Significant methodological, evaluation, or positioning problems that would require major rework. You would actively argue AGAINST acceptance at a PC meeting. The core idea may have merit but the execution is not there yet.
- `3 = Borderline`: The contribution is real and the work has genuine strengths, but notable weaknesses remain. You see the outcome as genuinely uncertain and dependent on what your co-reviewers find. You could be persuaded in either direction by new evidence or perspective.
